...
(function () {
  ...
  function updateUserGreeting() {
    ...
    if (!el) return;
    el.textContent = "";
    el.appendChild(document.createTextNode("Habit Tracker Pro "));
    if (currentUser) {
      const s = document.createElement("span");
      s.className = "user-greeting";
      s.textContent = `👋 ${currentUser.toUpperCase()}`;
      el.appendChild(s);
    }
  }
  ...
  function calcBestStreak(habitId) {
    // FIX: Include freeze dates in the calculation
    const completedDates = Object.keys(state.completions[habitId] || {}).filter(
      (d) => state.completions[habitId][d],
    );
    const frozenDates = state.freezeUsedDates || [];
    const allDates = [...new Set([...completedDates, ...frozenDates])].sort();
    ...
    allDates.forEach((ds) => {
      const d = new Date(ds);
      if (prev) {
        // FIX: Math.round prevents DST 23-hour day bugs
  ...
  // ─────────────────────────────────────────
  // CONFETTI + SOUND (FIXED FLICKER)
  // ─────────────────────────────────────────
  let _confettiFrameId = null;
  function triggerConfetti() {
    const canvas = document.getElementById("confettiCanvas");
    if (!canvas) return;

    // FIX: Cancel existing animation to prevent overlap/flickering
    if (_confettiFrameId) cancelAnimationFrame(_confettiFrameId);
    ...
    function draw() {
      ...
      if (++frame < 120) {
        _confettiFrameId = requestAnimationFrame(draw);
      } else {
        ctx.clearRect(0, 0, canvas.width, canvas.height);
        canvas.hidden = true;
      }
  ...
  function generateStreaksAndBadges() {
    ...
    title.textContent = "🔥 Current Streaks";
    ...
    if (bestEl) bestEl.textContent = maxS;
  ...
  function generateWeekView() {
    ...
    getVisibleHabits().forEach((h) => {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        html += `<td><div class="checkbox${chk ? " checked" : ""}${frz ? " frozen" : ""}" data-toggle="${h.id}" data-date="${ds}"></div></td>`;
      }
      html += "</tr>";
    });

    // FIX: Added missing summary rows for week view
    [
  ...
  function generateMentalStateGrids() {
    ...
    const dg = document.getElementById("dayNumberGrid");
    const mg = document.getElementById("moodGrid");
    const motg = document.getElementById("motivationGrid");
    if (dg) dg.innerHTML = dayH;
    if (mg) mg.innerHTML = moodH;
    if (motg) motg.innerHTML = motH;
  }
  ...
  // ─────────────────────────────────────────
  // CHARTS & NAVIGATION (FIXED WEEK VIEW SYNC)
  // ─────────────────────────────────────────
  ...
  function changeMonth(dir) {
    if (state.calendarView === "week") {
      ...
      // FIX: Sync month and year when flipping through weeks
      state.currentMonth = w.getMonth();
      state.currentYear = w.getFullYear();
    } else {
    ...
    habits.forEach((h) => {
      ...
      html += `<div class="ring-item">
        <svg width="72" height="72" viewBox="0 0 72 72">
          ...
          <circle cx="36" cy="36" r="${r}" fill="none" stroke="${cat.accent}" stroke-width="6" stroke-dasharray="${circ.toFixed(1)}" stroke-dashoffset="${offset.toFixed(1)}" stroke-linecap="round" transform="rotate(-90 36 36)" style="transition:stroke-dashoffset 0.6s ease"/>
  ...
  function openBulkCheckin() {
    ...
    state.habits.forEach((h) => {
      ...
      text.textContent = `${h.emoji} ${h.name}`;
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    document
      .getElementById("openBulkBtnHeader")
    ...
    const motGrid = document.getElementById("motivationGrid");
    if (motGrid) {
      motGrid.addEventListener("click", (e) => {
