...
(function () {
  ...
  function changeMonth(dir) {
    if (state.calendarView === "week") {
      const w = new Date(
        state.currentWeekStart || weekStart(new Date()).toISOString(),
      );
      w.setDate(w.getDate() + dir * 7);
      state.currentWeekStart = w.toISOString(); // Fix #17: always ISO string
    } else {
    ...
    // Fix #19: Do NOT reset bestMonthScore — scores are stored per month in bestMonthScores
    generateCalendar();
    updateStats();
    refreshCharts();
    generateAnalysis();
  ...
  function init() {
    ...
    // Auto-scroll to yesterday's column so user can log missed habits immediately
    requestAnimationFrame(scrollToYesterday);
  }
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("weekViewCalBtn").addEventListener("click", () => {
      ...
      if (!state.currentWeekStart)
        state.currentWeekStart = weekStart(new Date()).toISOString(); // Fix #17: store ISO string
      saveData();
      syncCalendarViewButtons();
      generateCalendar();
    });
